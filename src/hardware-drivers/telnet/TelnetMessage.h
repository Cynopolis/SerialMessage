...
public:
  ...
  void Init(uint32_t baudRate) override {
    Serial.println("Never call this without a parameter, this will not "
                   "function properly without a callback function");
  }
  /**
...
void TelnetMessage<SERIAL_BUFFER_SIZE, MAX_ARGS,
                   MAX_CALLBACKS>::onConnectCallback(String ip) {
...
template <uint32_t SERIAL_BUFFER_SIZE, uint32_t MAX_ARGS,
          uint32_t MAX_CALLBACKS>
void TelnetMessage<SERIAL_BUFFER_SIZE, MAX_ARGS,
                   MAX_CALLBACKS>::onConnectionAttemptCallback(String ip) {
  GlobalPrint::Print("- Telnet: ");
  GlobalPrint::Print(ip);
...
template <uint32_t SERIAL_BUFFER_SIZE, uint32_t MAX_ARGS,
          uint32_t MAX_CALLBACKS>
void TelnetMessage<SERIAL_BUFFER_SIZE, MAX_ARGS,
                   MAX_CALLBACKS>::onReconnectCallback(String ip) {
  GlobalPrint::Print("- Telnet: ");
  GlobalPrint::Print(ip);
...
template <uint32_t SERIAL_BUFFER_SIZE, uint32_t MAX_ARGS,
          uint32_t MAX_CALLBACKS>
void TelnetMessage<SERIAL_BUFFER_SIZE, MAX_ARGS,
                   MAX_CALLBACKS>::onDisconnectCallback(String ip) {
  GlobalPrint::Print("- Telnet: ");
  GlobalPrint::Print(ip);
